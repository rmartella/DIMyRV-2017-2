...
public:
	...
	InputManager() :
			lastMousePos(glm::ivec2(0.0)), camera(nullptr), readPixelBuffer(
					false) {
	...
	bool* getMouseButtomState() {
		return mouseButtomState;
	}

	bool isReadPixelBuffer() {
		return readPixelBuffer;
	}

	void setReadPixelBuffer(bool readPixelBuffer) {
		this->readPixelBuffer = readPixelBuffer;
	}

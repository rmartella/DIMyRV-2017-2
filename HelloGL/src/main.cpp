...
void cursorPosCallback(GLFWwindow* window, double x, double y) {
	CEGUI::System::getSingleton().getDefaultGUIContext().injectMousePosition(x,
			y);
}
...
void windowResizedCallback(GLFWwindow* window, int width, int height) {
	CEGUI::System::getSingleton().notifyDisplaySizeChanged(
			CEGUI::Sizef(static_cast<float>(width),
...
void initWindows() {
	...
	// load layout
	Window* root = WindowManager::getSingleton().loadLayoutFromFile(
			"TabPage.layout");
	System::getSingleton().getDefaultGUIContext().setRootWindow(root);

	CEGUI::Window *myImageWindow =
			CEGUI::WindowManager::getSingleton().createWindow(
					"TaharezLook/StaticImage", "PrettyWindow");
	myImageWindow->setPosition(
			CEGUI::UVector2(CEGUI::UDim(0.5, 0), CEGUI::UDim(0.5, 0)));
	myImageWindow->setSize(
			CEGUI::USize(CEGUI::UDim(0, 150), CEGUI::UDim(0, 100)));
	myImageWindow->setProperty("Image", "TaharezLook/PopupMenuFrameTopLeft");
	CEGUI::System::getSingleton().getDefaultGUIContext().getRootWindow()->addChild(
			myImageWindow);
}
...
int main(int argc, char** argv) {
	...
	OpenGL3Renderer* renderer =
			static_cast<OpenGL3Renderer*>(System::getSingleton().getRenderer());
	...
	while (glfwWindowShouldClose(window) == GL_FALSE) {
		...
		System::getSingleton().getDefaultGUIContext().injectTimePulse(
				time_elapsed);
		time = newtime;
